...
var Outside = {
	...
	TrapDrops: [
		...
		},
		{
			rollUnder: 0.995,
			name: 'ripped cloth',
			message: _('tattered ripped cloth')
		},
		{
			rollUnder: 1.0,
			name: 'charm',
	...
	init: function(options) {
		...
		this.tab = Header.addLocation(_("A Silent Wasteland"), "outside", Outside);
	...
	getMaxPopulation: function() {
		return $SM.get('game.buildings["refuge"]', true) * Outside._HUT_ROOM;
	},
	...
	destroyHuts: function(num, allowEmpty) {
		...
		for(var i = 0; i < num; i++){
			...
			var huts = (allowEmpty) ? $SM.get('game.buildings["refuge"]', true) : Math.ceil(rate);
			if(!huts) {
				break;
			}
			...
			$SM.set('game.buildings["refuge"]', ($SM.get('game.buildings["refuge"]') - 1));
			if(inhabitants){
	...
	updateVillage: function(ignoreStores) {
		...
		var hasPeeps;
		if($SM.get('game.buildings["refuge"]', true) === 0) {
			hasPeeps = false;
	...
	checkWorker: function(name) {
		var jobMap = {
			'hunting outpost': ['hunter', 'trapper'],
			'tannery': ['tanner'],
	...
	setTitle: function() {
		var numHuts = $SM.get('game.buildings["refuge"]', true);
		var title;
		if(numHuts === 0) {
			title = _("A Silent Wasteland");
		} else if(numHuts == 1) {
			title = _("A Lonely refuge");
		} else if(numHuts <= 4) {
	...
	onArrival: function(transition_diff) {
		...
		if(!$SM.get('game.outside.seenForest')) {
			Notifications.notify(Outside, _("the sky is grey, you can see the remnants of the nuclear falout"));
		...
		// set music
		var numberOfHuts = $SM.get('game.buildings["refuge"]', true);
		if(numberOfHuts === 0) {
	...
	gatherWood: function() {
		Notifications.notify(Outside, _("you found some scraps laying around outside"));
		var gatherAmt = $SM.get('game.buildings["cart"]', true) > 0 ? 250 : 50;
		$SM.add('stores.scrap', gatherAmt);
	...
	checkTraps: function() {
		var drops = {};
		var msg = [];
		var numTraps = ($SM.get('game.buildings["trap"]', true)*5);
		var numBait = $SM.get('stores.bait', true);
		...
		var s = _('the traps contain testtestetse');
