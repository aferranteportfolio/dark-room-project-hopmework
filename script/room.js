...
var Room = {
	...
	_NEED_wood_DELAY: 15 * 1000, // from when the stranger shows up, to when you need scrap
	buttons: {},
	Craftables: {
		'trap': {
			name: _('trap'),
		...
		'cart': {
			...
			button: null,
			maximum: 1,
			availableMsg: _('builder says she can make a cart for carrying scrap'),
			buildMsg: _('the rickety cart will carry more scrap from the forest'),
			type: 'building',
			cost: function () {
				return {
					'scrap': 30
		...
		'refuge': {
			...
			buildMsg: _('builder puts up a refuge, out in the forest. says word will get around.'),
			maxMsg: _('no more room for huts.'),
			type: 'building',
			cost: function () {
				var n = $SM.get('game.buildings["refuge"]', true);
				return {
					'scrap': 100 + (n * 50)
				};
			},
		...
		'hunting outpost': {
			name: _('hunting outpost'),
			button: null,
			maximum: 1,
			availableMsg: _('villagers could help hunt, given the means'),
			buildMsg: _('the hunting hunting outpost stands in the forest, a ways out of town'),
			type: 'building',
			cost: function () {
				return {
					scrap: 200,
	...
	init: function (options) {
		...
		if (Engine._debug) {
			...
			this._NEED_mainResourceVault_DELAY = 5000;
		}
		...
		if ($SM.get('game.builder.level') == 1 && $SM.get('stores.scrap', true) < 0) {

			Engine.setTimeout(Room.unlockForest, Room._NEED_mainResourceVault_DELAY);
		}
		Engine.setTimeout($SM.collectIncome, 1000);

		Notifications.notify(Room, _("the vault is {0}", Room.TempEnum.fromInt($SM.get('game.temperature.value')).text));
	...
	setTitle: function () {
		var title = $SM.get('game.fire.value') < 2 ? _("A Dark Vault") : _("A Firelit Vault");
	...
	_tempTimer: null,
	lightFire: function () {
		var scrap = $SM.get('stores.scrap');
		if (scrap < 5) {
			Notifications.notify(Room, _("not enough scrap to get the fire going"));
			Button.clearCooldown($('#lightButton.button'));
			return;
		} else if (scrap > 4) {
			$SM.set('stores.scrap', scrap - 5);
		}
	...
	stokeFire: function () {
		...
		var scrap = $SM.get('stores.scrap');
		if (scrap === 0) {
			Notifications.notify(Room, _("the scrap has run out"));
			Button.clearCooldown($('#stokeButton.button'));
			return;
		}
		if (scrap > 0) {
			$SM.set('stores.scrap', scrap - 1);
		}
	...
	coolFire: function () {
		...
		if ($SM.get('game.fire.value') <= Room.FireEnum.Flickering.value &&
			$SM.get('game.builder.level') > 3 && scrap > 0) {
	...
	adjustTemp: function () {
		...
		if ($SM.get('game.temperature.value') > 0 && $SM.get('game.temperature.value') > $SM.get('game.fire.value')) {
			...
			Notifications.notify(Room, _("the vault is {0}", Room.TempEnum.fromInt($SM.get('game.temperature.value')).text), true);
		}
	...
	unlockForest: function () {
		$SM.set('stores.scrap', 4);
		Outside.init();
		Notifications.notify(Room, _("the wind howls outside"));
		Notifications.notify(Room, _("the scrap is running out"));
	...
	updateStoresView: function () {
		console.log($SM.get('game.builder.level'))
	...
	craftUnlocked: function (thing) {
		...
		// Show buttons if we have at least 1/2 the scrap, and all other components have been seen.
		if ($SM.get('stores.scrap', true) < cost['scrap'] * 0.5) {
			return false;
		}
		for (var c in cost) {
			if (!$SM.get('stores["' + c + '"]')) {
